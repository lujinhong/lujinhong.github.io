...
##（七）异常处理
1、java.lang.NoClassDefFoundError: Could not initialize class com.yammer.metrics.Metrics

其实,noClassDefFound一般都是以下2个原因：
（1）classpath中真的没有这个包，她就是STORM_HOME/lib下没有metrics-core-2.2.0.jar包。
（2）classpath中有多个不同版本的metrics包，导致冲突，storm不知道应该用哪个。
但这里2种都不是，继续google，还有可能是
（1）kafka的broker没启动，发现不是
（2）zk没启动，就是本机上的zk没启动，
